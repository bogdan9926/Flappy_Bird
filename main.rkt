...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (append pipes (list (pipe-struct (+ pipe-width pipe-gap (pipe-struct-x (car (take-right pipes 1)))) (+ added-number (random random-threshold)))))
      pipes))
